...
export default async function GalleryComponent({ galleryId } : { galleryId: number}) {
  const gallery = galleries[galleryId - 1];
  const columns: _Object[][] = [];
  const galleryImageData = await imageHelper.getImageData(`gallery:${gallery.heroImageTag}`);
